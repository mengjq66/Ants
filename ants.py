...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            "*** YOUR CODE HERE ***"
            if type(insect) == QueenAnt and insect.queen_count == 1:
                return
            elif insect.container == True and insect.ant != None:
                self.ant = insect.ant
            else:
                self.ant = None
        else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        "*** YOUR CODE HERE ***"
        # return False if either there is no Ant in the Bee's place
        # or if there is an Ant, but its blocks_path attribute is False.
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        while True:
            if current_place == hive or current_place.entrance == None:
                return
            random_bee = random_or_none(current_place.bees)
            if random_bee and self.min_range <= distance <= self.max_range:
                return random_bee
            else:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            ...
            eated_bee = random_or_none(self.place.bees)
            self.eat_bee(eated_bee)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        QueenAnt.total_queen_count += 1
        self.queen_count = QueenAnt.total_queen_count



    def do_action(self, direction):
        current_place = self.place
        if current_place.ant not in self.doubled_list:
            ...
            if current_place.ant != None:
                if current_place.ant not in self.doubled_list:
                    if current_place.ant.container == True:
                        ...
                        self.doubled_list.append(current_place.ant)
                        self.doubled_list.append(current_place.ant.ant)
                    else:
                        current_place.ant.damage *= 2
                        self.doubled_list.append(current_place.ant)
